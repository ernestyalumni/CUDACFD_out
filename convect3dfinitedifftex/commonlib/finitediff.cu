...
__device__ float dev_div1( float3 stencil[1][2]  ) {
	...
	div_value += dev_dirder1( stencilz, c_nusz ) ;

	return div_value;
}

__device__ float dev_div2( float3 stencil[2][2]  ) {
	...
	div_value += dev_dirder2( stencilz, c_nusz ) ;

	return div_value;
}

__device__ float dev_div3( float3 stencil[3][2]  ) {
	...
	float stencilz[3][2] { { stencil[0][0].z, stencil[0][1].z }, { stencil[1][0].z, stencil[1][1].z }, { stencil[2][0].y, stencil[2][1].y } };
	...
	div_value += dev_dirder3( stencilz, c_nusz ) ;

	return div_value;
}

__device__ float dev_div4( float3 stencil[4][2]  ) {
	...
	div_value += dev_dirder4( stencilz, c_nusz ) ;
