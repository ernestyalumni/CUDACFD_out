...
//								 0.0061035f , 
	...
	HANDLE_ERROR(
		cudaMemcpyToSymbol( dev_gas_params, gas_params, sizeof(float)*2,0,cudaMemcpyHostToDevice) );
...
//	set4DerivativeParameters(hds);
	...
	HANDLE_ERROR( 
		cudaMemcpy( dev_grid2d.dev_rho_out , grid2d.rho, grid2d.NFLAT()*sizeof(float), 
			cudaMemcpyHostToDevice)
	);
