...
__global__ void Euler2dp1( float* dev_rho, float2* dev_p, float2* dev_u, float* dev_E ) {
	...
	if (dev_rho[k] != 0.f ) {
		...
		if ( (isfinite( temp_u.x )) && (isfinite( temp_u.y) ) ) {
			dev_u[k] = temp_u  ;
		}
		else {
			;
...
__global__ void Euler2dp2( float* dev_rho, float2* dev_p, float2* dev_u, float* dev_E ) {
	...
	float div_value { dev_div2( rhoustencil ) } ;
	...
	float div_Eval { dev_div2( Estencil ) } ; 
	...
	float div_KEval { dev_div2( KEstencil ) } ; 
	...
	float div_pxval { dev_div2( pxstencil ) } ; 
	...
	float div_pyval { dev_div2( pystencil ) };
	...
	float2 grad_pressureval = dev_grad2( pressurestencil ) ;
