...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes, row sep=3.2em, column sep=2.8em, minimum width=2.4em] 
  {
u(k) \in \text{Hom}(\mathbb{Z},\mathbb{R}^3) &  \\ %((i_x,i_y,i_z),u(i_x,i_y,i_z)) \in \mathbb{Z}^3 \times \text{Hom}(\mathbb{Z}^3 , \mathbb{R}^3) \\ 
...
%$L_x,L_y,L_z$ in \verb|__constant__| memory, \verb|__constant__ int dev_Ld[3]; |
