...
// dynamics (parameters)
const dim3 M_i { 64, 64 }; 	// number of threads per block, i.e. Niemeyer's BLOCK_SIZE // I've tried values 4,4
...
// graphics
/* ----------------------------------------------------------------- */
const int iters_per_render { 100 };
...
void make_render(dim3 Ld_in, int iters_per_render_in, GPUAnim2dTex* texmap ) {
	...
	// calculate time step based on stability and CFL
		...
		sum_pressure <<<grid_pr, block_pr>>> (dev_grid2d.pres_red, dev_grid2d.pres_black, pres_sum_d, 
		...
		std::cout << "Time = " << t+deltat << ", deltat = " << deltat << ", iter = " <<
			iter << ", res (i.e. norm_L2) = " << norm_L2 << std::endl;
		
		// calculate new velocities and transfer maximums back

		calculate_u <<<grid_pr, block_pr>>> ( dev_grid2d.F, dev_grid2d.pres_red, dev_grid2d.pres_black, 
			dev_grid2d.u, max_u_d,
			deltat, grid2d.hd[0], grid2d.hd[1], grid2d.Ld[0], grid2d.Ld[1], M_i.x);
		cudaMemcpy (max_u_array.data(), max_u_d, size_max * sizeof(float), cudaMemcpyDeviceToHost);

		calculate_v <<<grid_pr, block_pr>>> (dev_grid2d.G, dev_grid2d.pres_red, dev_grid2d.pres_black, 
